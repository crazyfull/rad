...
#include "libRadiusClient/RadiusServerStack.h"
 ...
 *   - it answers only Access-Requests with user="nemo" and password="gooz"
 */
...
int verifyRequest(RadiusPacket &p_request)
{
    ...
    {
        printf("ERROR: Wrong request code(%s)\n", p_request.getCodeDescription());
        return RC_FAIL;
    }
    ...
    if ((memcmp(l_password, "gooz", 4) != 0))
    {
        printf("ERROR: Wrong User-Password attribute\n");
        return RC_FAIL;
    }
...
#ifdef _MSC_VER
    l_addr.S_un.S_addr = inet_addr("192.168.1.3");
#else
...
int main2 (int argc, char *argv[])
{
    ...
    RadiusServerStack l_stack( RadiusSecret("moji"), 1812);
    if (!l_stack.isValid())
    {
